courbes.prop3 <-
function(datas.GrapheR2,nb.niv,var1,fact,nivx,var2,niv1,err.type,lig,tra,x.inf,x.sup,y.inf,y.sup,log.axes,x.legende,y.legende,
  taille.legende,col.legende,col.symb,symbol,taille.symb,ep.lig,err.seg,pos.leg,leg.titre,noms,leg) {
  ...
  for(i in 1:nb.niv) {
    ...
    liste.proportions[[i]]=tapply(y,x,function(x) length(x[x==niv1])/length(x))
    liste.err=calc.err4(err.type=err.type,x=x,y=y,niv1=niv1,proportions=liste.proportions[[i]])
  ...
  max=integer(length(liste.proportions))
  min=integer(length(liste.proportions))
  for (i in 1:length(liste.proportions)) {
    max[i]=max(liste.proportions[[i]]+liste.barres.sup[[i]])
    min[i]=min(liste.proportions[[i]]-liste.barres.inf[[i]])
  }
  ...
  if (nchar(err.type)>0 & err.type!=Env$vocab[118,1]) {courbes.erreurs2(liste.val=liste.proportions,liste.barres.inf=liste.barres.inf,
